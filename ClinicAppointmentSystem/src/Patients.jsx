        ...
        {patients.length === 0 ? (
          ...
          >
            ...
            <div style={{ fontSize: "13px", marginTop: "8px" }}>
              Patients who sign up will appear here
            </div>
          </div>
        ) : (
          patients.map((p) => {
            const patientAppts = getAppointments(p.name);
            const approvedCount = patientAppts.filter(
              (a) => a.status === "Approved"
            ).length;
            const rejectedCount = patientAppts.filter(
              (a) => a.status === "Rejected"
            ).length;
            const scheduledCount = patientAppts.filter(
                ...
                {patientAppts.length > 0 && (
                  ...
                  >
                    <details style={{ cursor: "pointer" }}>
                      ...
                      >
                        📅 View {patientAppts.length} Appointment
                        {patientAppts.length !== 1 ? "s" : ""} ({scheduledCount}{" "}
                        Scheduled)
                      </summary>
