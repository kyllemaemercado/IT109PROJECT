...
const DoctorPatientRecords = ({ user }) => {
  ...
  const doctorAppointments = useMemo(() => {
    return appointments.filter(
      (a) =>
        a.providerName === user?.name &&
        a.providerRole === user?.role
    );
  }, [appointments, user]);
  ...
  const patientRecords = useMemo(() => {
    ...
    return Array.from(patientNames).map((patientName) => {
      const patientAppts = doctorAppointments.filter((a) => a.patientName === patientName);
      ...
      return {
        ...
        appointments: patientAppts.sort((a, b) => new Date(b.date) - new Date(a.date)),
        approvedCount: patientAppts.filter((a) => a.status === 'Approved').length,
        rejectedCount: patientAppts.filter((a) => a.status === 'Rejected').length,
        completedCount: patientAppts.filter((a) => a.status === 'Completed').length,
  ...
  if (patientRecords.length === 0) {
    return (
      <div style={{ padding: '40px', textAlign: 'center', color: '#6B7280' }}>
        ...
        <p style={{ fontSize: '16px', fontWeight: '600' }}>No patient records found</p>
              ...
              >
                ...
                >
                  <div style={{ textAlign: 'left', flex: 1 }}>
                    ...
                    >
                      Total Appointments: {patient.totalAppointments} • Last Visit: {patient.lastVisit}
                    </p>
                  </div>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  ...
                                  {appt.status === 'Cancelled' && '🚫'} {appt.status}
                                </span>
                              </td>
                              <td
                                style={{
